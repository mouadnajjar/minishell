...
/*   Updated: 2025/05/09 16:43:31 by ahlahfid         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_token	*create_token(t_token_type type, const char *value, size_t len)
{
	...
	token->type = type;
	token->value = gc_malloc(&gc, len + 1); // Allocate space for null terminator
	if (!token->value)
		return (NULL);
...
t_token	*lexer(const char *input)
{
	...
	while (input[i])
	{
		...
		if (!head)
			...
			current->next = token;
		current = token;
		// Remove unnecessary increment of `i` here
	}
	return (head);
}
